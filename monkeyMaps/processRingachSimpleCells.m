...
for i = 1:10
    for j = 1:25
        ...
        sf = Allparam(k,4).*Allparam(k,6);
        title(num2str(round(4*sf*1000)/1000))
...
%         %Convert units to degrees
        ...
        [Allparam(k,:) Allffit{k} Allvaraccount(k)] = Gaborfit2Drot_nophase(rf(k).rf,rf(k).d2p);
        ...
        nx = 4*Allparam(k,2)*Allparam(k,4);
        title(num2str(round(nx*1000)/1000))
...
%         [param  varacc pref sig sfMarg domMarg ffit domII] = sfFitfrom2D(rfF,rf(k).d2p);
        ...
        [sfpref sfsig orisig varacc_sf varacc_ori] = sforiFitfrom2D_2(rfF,rf(k).d2p,0,sfmodel);
...
%sfpref = AllparamGuess(id,6);
a2p = (1./(2*TCroAll.sfpref));
b2p = (TCopAll.profileSize*2); %actual RF width
%b2p = TCopAll.ysize*2;
...
xlabel('1/sfreq preference'), ylabel('RF size; 4 sigma'),% xlim([0 3])
title(['r=' num2str(r(1,2)) '; p=' num2str(p(1,2))])

%[param ffit varaccount] = Sigfit(a,b)

[param ffit varaccount dom] = NLsat(a,b,[.5 1])
