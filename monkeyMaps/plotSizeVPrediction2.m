function plotSizeVPrediction2(Rsize,Rsizepred,scatterdom)
...
[param ffit dom varaccount] = LinetotalLS((Rsizepred),(Rsize),[1 0]);
param


% H = [Rsizepred ones(size(Rsizepred))];
% y = Rsize;
% param = inv(H'*H)*H'*y
% dom = linspace(H(end,1),H(end,1),100)


hold on
dom = 2.^hdom;
loglog(dom,(dom*param(1)+param(2)),'--g')
...
title(['geomean(ratio) = ' num2str(2.^nanmean(predratio)) '; p=' num2str(p)])
