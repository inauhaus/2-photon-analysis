function [XsigVsf BWLinVsf BWVsf sfpref F1vsBand] = V1_ComplexCell_generation(sfBands,xsigDom,sfsigDom,sf2sigModel)
...
xN = 300;
sfN = 300;
xCenterdom = linspace(-2,2,xN);
%sfprefdom = (logspace(log10(.1),log10(10),sfN)); 
...
[sfprefmesh xCentermesh xmesh] = ndgrid(sfprefdom,xCenterdom,xdom); %y x z
id = find(sfprefmesh(:)>8 | sfprefmesh(:)<.25);
boundaryMask = ones(size(sfprefmesh));
boundaryMask(id) = 0;
...
%SimplePop = Env.*cos(xmesh.*sfprefmesh*2*pi); %constant absolute phase
%SimplePop2 = Env.*sin(xmesh.*sfprefmesh*2*pi);

SimplePop = Env.*cos((xmesh-xCentermesh).*sfprefmesh*2*pi); %constant relative phase
...
%SimplePop = SimplePop./sigmesh;
%SimplePop2 = SimplePop2./sigmesh;

SimplePop = SimplePop.*boundaryMask;
SimplePop2 = SimplePop2.*boundaryMask;
...
for i = 1:20:sfN
    ...
    Env1 = squeeze(Env(i,round(size(Env,2)/2),:));
    plot(xdom,Env1,'k')
    hold on
end
...
cdom = jet;
id = round(linspace(1,length(cdom(:,1)),length(sfBands)));
cdom = cdom(id,:);
...
for k = 1:length(sfsigDom)
    ...
    for j = 1:length(xsigDom)
        ...
        for i = 1:length(sfBands)
            ...
            %cyc/deg
            Gweight = exp(-xCentermesh.^2/(2*xsig^2)) .* exp(-(sfprefmesh-sfBands(i)).^2/(2*sfsig^2));
            
            %imagesc(xCenterdom,sfprefdom,Gweight(:,:,50)),
            figure(87)
            contour(xCenterdom,sfprefdom,Gweight(:,:,50),[.61 61],'Color',cdom(i,:)),
            ...
            RFout = sqrt(squeeze(sum(sum(In,1),2)));
            RFout = RFout/max(RFout);

            %Get sf tuning curve for each cell (F0)
            %TCin = abs(fft(Gweight.*SimplePop,[],3)) + abs(fft(Gweight.*SimplePop2,[],3)); %Mag of response of each neuron to each spatial frequency. e.g. Number of spikes it passes downstream.            
            %F0out = squeeze(sum(sum(TCin,1),2)); %Now add up the spikes for each spatial frequency     
            
            %Think about it as the response to a drifting grating... the
            %summed response over phase. Then you just add up the
            %magnitudes from each cell. 
            TCin = abs(fft(Gweight.*SimplePop2,[],3)); 
            F0out = squeeze(sum(sum(TCin,1),2)); %Now add up the spikes for each spatial frequency
            
            F0out = F0out/pi; %half wave rectification
            
            %Get F1 modulation for each cell
            F1in = fft(Gweight.*SimplePop2,[],3);
            F1out = abs(squeeze(sum(sum(F1in,1),2))); %add up the vectors. High sfs will interfere deconstructively... low sfs will be construct.
            ...
            XsigVsf(i,j,k) = idsig*(xdomI(end)-xdomI(end-1));
            
            XsigVsf(i,j,k) = sqrt(sum(RFoutI.*xdomI.^2)/sum(RFoutI));  
...
%             dsf = TCdomI(end)-TCdomI(end-1);
            BWLinVsf(i,j,k) = (sfhi-sflo)/2; %sigma
            %BWLinVsf(i,j,k) = (sfhi-sfpref(i,j,k)); %sigma
            BWVsf(i,j,k) = (sfhi/sflo);
            
            com = sum(TCoutI.*(TCdomI))/sum(TCoutI);
            BWLinVsf(i,j,k) = (sum(TCoutI.*(TCdomI-com).^2)/sum(TCoutI)).^.5;
