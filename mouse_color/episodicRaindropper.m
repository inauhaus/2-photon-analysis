function rst = episodicRaindropper(posdom)

global cellS ACQinfo Analyzer
...
for pid = 2:length(kern)
    ...
    %kern{pid}(1) = kern{pid}(1);
    %kern{pid}(2) = kern{pid}(2);
    
    rst.sOFF.kern(p,:) = kern{pid}(4,:)';
    rst.sON.kern(p,:) = kern{pid}(2,:)';
    rst.mOFF.kern(p,:) = CR*kern{pid}(3,:)';
    rst.mON.kern(p,:) = CR*kern{pid}(1,:)';
    ...
    %plot(kern{p}')
    thsh = .3;
    if rst.sOFF.varacc<thsh || rst.sON.varacc<thsh || rst.mOFF.varacc<thsh || rst.mON.varacc<thsh
        %if rst.sOFF.varacc<thsh & rst.sON.varacc<thsh & rst.mOFF.varacc<thsh & rst.mON.varacc<thsh
        %if rst.Env.varacc<thsh
    ...
    figure(99)
    subplot(ceil(sqrt(NC)),ceil(sqrt(NC)),p)
    ...
    figure(101)
    subplot(ceil(sqrt(NC)),ceil(sqrt(NC)),p)
    plot(posdom,rst.mON.kern(p,:)-rst.mOFF.kern(p,:),'.g'), hold on,
    hold on
    plot(posdom,rst.sON.kern(p,:)-rst.sOFF.kern(p,:),'.b'),
    hold on
    ...
    title(num2str(rst.Env.param(p,1)));
...
rst.EnvM.param(:,1) = rst.EnvM.param(:,1) + posdom(1);
...
Winmap = zeros(length(posdom),Ngood);
funcmap = zeros(length(posdom),Ngood);
...
for pdum = 1:Ngood 
    ...
    Win = rst.Env.ffit(p,:);
    Win = Win-min(Win);
    Win = Win./(rst.Env.param(p,3)+rst.Env.param(p,4));
    %Win = Win-min(Win);
    %Win = Win/max(Win);
...
%    H = mN(good,v);
%    y = sN(good,v);
%    SRratio(v) = inv(H'*H)*H'*y; %used for nonlinearity
% end

%Or just take the average of the ratio
RetEnvelope = nanmedian(sN + mN)/2;
id = find(~isnan(sN(:,1)));
SEnvelope = geomean(sN(id,:));
MEnvelope = geomean(mN(id,:));
...
for i = 1:Ngood    
    id = find(~Winmap(:,i));
    funcmap2(id,i) = pS_hat(id);    
end

figure,imagesc(1:Ngood,posdom,funcmap2), 

colormap(SMmap)
caxis(clim)

%overlay the rectangles
dpos = (posdom(2)-posdom(1));
for i = 1:Ngood
   id = find(Winmap(:,i)); 

   yp = posdom(id(1));
   yw = length(id)*dpos;
   rectangle('Position',[i-.5 yp-dpos/2 1 yw])
...
figure,plot(posdom(3:11),pS_hat(3:11),'-ok'), ylim([0 1])
%% Nonlinearity

SRratio(find(1-Win)) = NaN;
[SWang] = getRetinaGradient(posdom);
figure,plot(SWang./(100-SWang),SRratio,'-o');
xlabel('O(v)/[1-O(v)]'), ylabel('[O(v)/[1-O(v)]]^N^L')
...
%% Plot estimate of weighting function of each cell w(v) = R(v)/O(v)
...
for pdum = 1:Ngood
    p = posid(pdum);
    subplot(ceil(sqrt(NC)),ceil(sqrt(NC)),pdum);
...
for pid = 2:length(kern)
    ...
    dum = corrcoef(RSOFF(p,:),RMOFF(p,:));
    projR(p) = dum(1,2);
    dum = corrcoef(wSOFF(p,:),wMOFF(p,:));
    projW(p) = dum(1,2);
...
vertRet = rst.Env.param(:,1);

vertplot = (vertRet-vrange(1))/(vrange(2)-vrange(1)) ;
...
ES = mean(rst.EnvS.ffit,2);
EM = mean(rst.EnvM.ffit,2);
ESpk = max(rst.EnvS.ffit,[],2);
EMpk = max(rst.EnvM.ffit,[],2);
%ESpk = rst.EnvS.param(:,3);
%EMpk = rst.EnvM.param(:,3);
%EMpk = phi(max(rst.EnvM.ffit,[],2));
%ESpk = phi(max(rst.EnvS.ffit,[],2));
pS = ES./(EM+ES);
pS(find(pS>1 | pS<0)) = NaN;
pSpk = ESpk./(EMpk+ESpk);
pSpk(find(pSpk>1 | pSpk<0)) = NaN;
...
subplot(2,2,4),
scatter(vertRet,pS,'k')
hold on
scatter(vertRet,pSpk,'r')


id = find(~isnan(pS.*vertRet));
[r p] = corrcoef(pS(id),vertRet(id,1))
r = round(r(1,2)*100)/100;
p = round(p(1,2)*100)/100;
[rpk ppk] = corrcoef(pSpk(id),vertRet(id,1))
rpk = round(rpk(1,2)*100)/100;
ppk = round(ppk(1,2)*100)/100;

legend(['integral; r=' num2str(r) '; p=' num2str(p)],['peak amplitude; r=' num2str(rpk) '; p=' num2str(ppk)])
