...
for i = 1:length(cellS.mukern)   
   ...
   i
    ...
    kdumF1 = cellS.F1kern{i};
    kdumF1 = permute(kdumF1,dimsort);  %force the kernel to have dimensions in this order
    ...
    for c = 1:2 %loop each color
        ...
        sforimatF1 = squeeze(kdumF1(cid(c),:,:));
        rst.sforimatF1{c}(:,:,i) = squeeze(mean(sforimatF1,3));
        ...
        %[SFparams ffitIsf domI domII pfit Gvaracc repFitQuality] = analyzeLINtuning_DoG2(sftc(:)',sfdom,cothresh);
       ...
       %Don't do this if you want to see the phase 
%         idy2 = idy+length(oridom)/2;
%         if idy2>length(oridom)
%             idy2 = idy2-length(oridom);
%         end
%         rst.tcourseOpt{c}(i,:) = (rst.tcourseOpt{c}(i,:) + squeeze(kdumT(c,idy2,idx,:))')/2;
...
tdom =(0:(size(rst.tcourseOpt{1},2)-1))*fr/1000; 
figure
for i = 1:length(idExamp)
    
    dumim1 = rst.sforimat{1}(:,:,idExamp(i));
    dumim2 = rst.sforimat{2}(:,:,idExamp(i));
    immin = min([dumim1(:); dumim2(:)]);
    immax = max([dumim1(:); dumim2(:)]);
    
    subplot(length(idExamp),subplotCols,(i-1)*subplotCols+1)
    imagesc(sfdom,oridom,rst.sforimat{1}(:,:,idExamp(i)),[immin immax]), axis square
    set(gca,'YTick',[0 180])
    if i == length(idExamp)
        xlabel('SF (cyc/deg)')
    ...
    subplot(length(idExamp),subplotCols,(i-1)*subplotCols+2)
    imagesc(sfdom,oridom,rst.sforimat{2}(:,:,idExamp(i)),[immin immax]), axis square
    set(gca,'YTick',[0 180])
    colormap jet    
    if i == 1
       title('color 2') 
    ...
    subplot(length(idExamp),subplotCols,(i-1)*subplotCols+3)
...
%     set(gca,'XTick',[0 90 180])
    ...
    subplot(length(idExamp),subplotCols,(i-1)*subplotCols+4)
 
    semilogx((sfdom),rst.sftc{1}(idExamp(i),:),'ok')
    hold on
    semilogx((sfdomII),rst.sftcfit{1}(idExamp(i),:),'k')
    hold on
    semilogx((sfdom),rst.sftc{2}(idExamp(i),:),'or')
    hold on
    semilogx((sfdomII),rst.sftcfit{2}(idExamp(i),:),'r')    
    set(gca,'Xtick',sfdom)    
    ...
    ylim(yl)
%     
    subplot(length(idExamp),subplotCols,(i-1)*subplotCols+5)
    plot(tdom,rst.tcourseOpt{1}(idExamp(i),:),'k')
    hold on
    plot(tdom,rst.tcourseOpt{2}(idExamp(i),:),'r')
    hold on
    ...
    %title(num2str(sfpref(i)))
%     
    subplot(length(idExamp),subplotCols,(i-1)*subplotCols+6)
    plot(tdom,rst.tcourseLowSF{1}(idExamp(i),:),'k')
    hold on
    plot(tdom,rst.tcourseLowSF{2}(idExamp(i),:),'r')
    hold on
    plot([getparam('predelay') getparam('predelay')+getparam('stim_time')],[-.1 -.1],'k')
    xlabel('sec'), ylabel('dF/F')
    axis tight
