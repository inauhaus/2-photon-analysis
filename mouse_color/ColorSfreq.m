...
for i = 1:length(cellS.mukern)   
    ...
    for c = 1:2 %loop each color
        ...
        %sforimat = sforimat(1:length(oridom),:)/2 + sforimat(length(oridom)+1:end,:)/2;
        
        rst.sforimat{c}(:,:,i) = sforimat;
        
        %Compute tuning curves using a weighted average of the orthogonal
        %dimension
        sfdum = sum(sforimat); 
        sfdum = ones(size(sfdum));
        sfdum  = sfdum/sum(sfdum);        
        oritc = sum(sforimat.*(ones(length(oridom),1)*sfdum),2);
        
        oridum = sum(sforimat,2); 
        oridum = ones(size(oridum)); 
        ...
        sftcI = interp1(log2(sfdom),sftc,log2(sfdomI),'spline')';        
        %[paramdum ffit_sf varacc_sf ffitIsf domIsf pk sfBW sflco sfhco] = DoGfit(sftcI,sfdomI');
        
        [Gparam ffit Gvaracc] = Gaussfit2(log2(sfdomI),sftcI');
        Gparam(1) = min([log2(sfdom(end)) Gparam(1)]);
        Gparam(1) = max([log2(sfdom(1)) Gparam(1)]);
        sfBW = 2.^Gparam(2);
        sfhco = 2.^Gparam(2) + 2.^Gparam(1);
                
        ffitIsf = interp1(log2(sfdomI),ffit,log2(sfdomII),'spline')';  %This is just used for plotting
        
        %[dum sfid] = max(ffitIsf);
        %rst.sfpref{c}(i) = domIsf(sfid);
        
        [dum sfid] = max(sftc);
        %rst.sfpref{c}(i) = sfdom(sfid);
        rst.sfpref{c}(i) = 2.^Gparam(1);
...
for i = 1:length(idExamp)
    ...
    %plot(oridom,rst.oritc{1}(idExamp(i),:),'ok')
    polar(oridom*pi/180,rst.oritc{1}(idExamp(i),:))
    %hold on
    %plot([oridomII 180],[rst.oritcfit{1}(idExamp(i),:) rst.oritcfit{1}(idExamp(i),1)],'k')
    hold on
    %plot(oridom,rst.oritc{2}(idExamp(i),:),'or')
    polar(oridom,rst.oritc{2}(idExamp(i),:),'r')
    axis off
    %hold on
