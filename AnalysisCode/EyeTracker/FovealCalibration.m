...
for c = 1:getnoconditions
    for r = 1:getnorepeats(c)
        xposC{c,r} = [];
        yposC{c,r} = [];
    end
end

trange = [getparam('predelay')/1000 getparam('predelay')/1000+2000];
...
for t = 1:nT
    ...
    idSac =  find(tdomEye>trange(1) & tdomEye<trange(2));
...
ypos = (ypos(idSac));
    ...
    xposC{c} = [xposC{c} xpos(:)'];
    yposC{c} = [yposC{c} ypos(:)'];
    

end

%%
RGBid = round(linspace(1,64,9));
rgbmat = jet;

k = 1;
figure
for c = 1:length(xposC)
    
        %subplot(3,3,c)
        scatter(xposC{c},yposC{c},'.','markeredgecolor',rgbmat(RGBid(c),:))
        hold on
