function [kernPop kernSigPop blankMu blankSig] = getTCmat_color
...
if prod(size(maskS.bwCell)) == 1
    maskS.bwCell{2} = zeros(size(maskS.bwCell{1})); 
end
...
for idsym = 1:Nsym
    if strcmp('theta',Analyzer.loops.conds{1}.symbol{idsym})
        oriid = idsym;
    elseif strcmp('ori',Analyzer.loops.conds{1}.symbol{idsym})
        contid = idsym;
    else
