...
x0 = [muGuess sigGuess AmpGuess AmpDCguess sigGuess2]; %initial guess for paramter search
%G = [length(f)/2 1 0 0];
...
[param,dum] = fminsearch('SoGfitter_handle2',x0);
...
ffit = exp(-(domain-param(1)).^2/(2*param(2).^2)) + param(4)*exp(-(domain).^2/(2*param(5).^2));
ffit = ffit*param(3);
