...
%             end            
            blank = nanmean(condcellMat(bframe1:bframe2,:));
...
for q = 1:length(cdom)  %loop through each condition
    ...
    if bsflag
        cellS.muTime{i} = squeeze(nanmean(cellS.cellMat_norm{i},3)); %nanmean across repeats
        cellS.muRep{i} = squeeze(nanmean(cellS.cellMat_norm{i}(:,Frame1:Frame2,:),2)); %nanmean across time
    else
        cellS.muTime{i} = squeeze(nanmean(cellS.cellMat{i},3)); %nanmean across repeats
        cellS.muRep{i} = squeeze(nanmean(cellS.cellMat{i}(:,Frame1:Frame2,:),2)); %nanmean across time
    end
    ...
    cellS.mu{i} = nanmean(cellS.muTime{i}(:,Frame1:Frame2),2);  %nanmean across repeats and time window
    ...
    cellS.maxi{i} = max(muTimesmooth(:,Frame1:Frame2),[],2);  %nanmean across repeats; max over time window
...
%     cellS.sig{i} = std(dum,[],2)/sqrt(nr*nf); %std error across time and repeats
    ...
    %I think computing the nanmean across time first (as done below) is a
    ...
    if bsflag
        ...
        dum = squeeze(nanmean(cellS.cellMat_norm{i}(:,Frame1:Frame2,:),2));
    else
        cellS.sigTime{i} = std(cellS.cellMat{i},[],3)/sqrt(nr); %std error across repeats
        dum = squeeze(nanmean(cellS.cellMat{i}(:,Frame1:Frame2,:),2));
    end
