...
% for t = 1:getnotrials
% 
%     cellMat{t} = cellS_RemoveSingValues(cellMat{t},N_PC);
% t
% end
...
tic
    ...
    for trialid = 1:length(trialdom)
        ...
        sighp = str2num(get(G_RChandles.Hwidth,'string'));
        siglp = str2num(get(G_RChandles.Lwidth,'string'));
        
        
        hhi = fspecial('gaussian', size(tcourse),sighp/acqPeriod);
        hlo = fspecial('gaussian', size(tcourse),siglp/acqPeriod);
        hhi = abs(fft(hhi));
        hlo = abs(fft(hlo));
        %hlo = [];
        %hhi = [];
        tcourse = processTcourse_NaN(tcourse,hhi,hlo,1,acqPeriod);
                            ...
                            for i = 1:length(stimes)
                                ...
                                if ~isempty(idx1)
                                    ...
                                    if tpiece(1)>0 && tpiece(end)<length(tcourse)
                                        %base = nanmean(squeeze(tcourse(tbase)));
                ...
                for color = 1:length(colordom)
                    if ~isempty(cellS.kernAll{p}{ori,sf,phase,color}) %if it didn't show this grating
                        ...
                        if blankNorm
                            ...
                            %need to nanmean because some frames may have
                            %been "removed" do to Z-motion.
                            cellS.muTime{p}(ori,sf,phase,color,:) = squeeze(nanmean(cellS.kernAll{p}{ori,sf,phase,color},1));
                            cellS.sigTime{p}(ori,sf,phase,color,:) = nanstd(cellS.kernAll{p}{ori,sf,phase,color},[],1)/sqrt(N);
                        end
                    end
                end
            end
    ...
    if size(kernplot,1) == 1 %if there was only one orientation
        ...
        kernplot = squeeze(nanmean(kernplot(:,3:end,:,:,:),2));   %mean across spatial freqency
    end
    ...
    imagesc(round(taudom),oridom,real(kernplot))
