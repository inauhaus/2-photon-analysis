function plotRandPosMaps_dots_paper_modelPred
...
for c = 1:length(DM.colordom)
    ...
    %% Orientation
    figure, 
    plotDotMap(TC.OAng{c}{kElem},'hsv',MK.masklabel,MK.celldom(MK.nID),[0 180]);
    title(['ori randpos ']), 
   ...
   % hyp = 10; orig = -5;
    ...
    %% Retinotopy X
    figure
    plotDotMap(xposnorm,'jet',MK.masklabel,MK.celldom(MK.nID),posrangeX);
    ...
    %% Retinotopy Y
    figure
    plotDotMap(yposnorm,'jet',MK.masklabel,MK.celldom(MK.nID),posrangeY);
    ...
    %% RF size
    figure,    
    %posrange = sort(log2(1./(2*[.25 8]))); %Put it on the inverse scale of the spatial frequency domain
    posrange = log2([1/32 1]);
    %sfPrediction = 1./(2*sizevals);  %1/4sigma
    plotDotMap(log2(sizevals/2),'jet',MK.masklabel,MK.celldom(MK.nID),posrange); %Plot size as 2sigma
    title(['color ' num2str(c)]),  
    %plotEXcirc(idExamp,xdom,ydom)    
    ...
    plotDotMap(TC.BWdiff{c}{kElem},'jet',MK.masklabel,MK.celldom(MK.nID),posrange);
    title(['color ' num2str(c)]),  
    %plotEXcirc(idExamp,xdom,ydom)    
    posvec = linspace(0,1,5); 
    for i = 1:length(posvec)
