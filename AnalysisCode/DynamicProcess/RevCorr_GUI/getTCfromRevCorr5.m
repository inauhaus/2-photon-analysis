...
%         end
        ...
        %[paramdum ffit_sf{p} varacc_sf ffitIsf domIsf] = GaussTLineFit(DM.sfdomI,tcsfI)
        %[paramdum ffit_sf{p} varacc_sf ffitIsf domIsf] = SoGfit(DM.sfdomI,tcsfI); sfBWLin = 2*paramdum(2);        
        TC.sfBWLin{c}(p) = NaN;
        
        [RFenv RFsize RF] = getRFenvelope(tcsf,DM.sfdom); 
        TC.RFsizeFromIFFT{c}(p) = RFsize;
        ...
        if varacc_ori > .7 & varacc_sf > .7 & TC.SNR{c}(p) > .3
...
%             TC.flo{c}(p) = domIsf(flodum);  %low cutoff sfreq           
            ...
            [TC.flo{c}(p) TC.fhi{c}(p)] = gethcolco(domIsf,ffitIsf',0.61);

            TC.LPness{c}(p) = (ffitIsf(1)+phi(mi))/(ffitIsf(idma)+phi(mi)); 
             
            %[TC.flo{c}(p) TC.sfpref{c}(p) TC.fhi{c}(p)] = getDoGparams([paramdum 0],0.61);
...
%             TC.LPness{c}(p) = (ffitIsf(1))/(ffitIsf(idma));   
            ...
            TC.sfSig{c}(p) = paramdum(2);
            TC.sfBWLin{c}(p) = TC.fhi{c}(p)-TC.flo{c}(p); %bandwidth in cyc/deg from sum of Gaussian fit             
