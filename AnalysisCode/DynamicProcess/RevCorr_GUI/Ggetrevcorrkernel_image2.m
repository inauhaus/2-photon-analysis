function [kernIm kernblank countmat countmatblank] = Ggetrevcorrkernel_image2(trialdom,hh)
...
global ACQinfo Analyzer G_RChandles G_handles domains mbestall nbestall cellS
...
T
    CH = GetTrialData([1 0],trialdom(trialid));
    
    if get(G_handles.fastMotionFlag,'value');
        
        mbest = cellS.motionInfo.mbest{trialdom(trialid)};
        nbest = cellS.motionInfo.nbest{trialdom(trialid)};
        
        CH{1} = motionCorrect(CH{1},mbest,nbest);
        
    end
    
    %CH{1} = subtractTemporalPoly(CH{1},3);

    %%
    
    %append or truncate data tensor to match the size of the first.
    if ~isempty(hh)        
        s = size(CH{1});
        dL = s(3)-length(hh);
        if dL<0
