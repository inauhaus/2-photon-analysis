...
if Monkbit
    ...
    % delayWin = [100 maDel];
    delayWin = [0 200];  %assume the peak response is within this time window
else
    %For mouse
    kernTsig = 50;
    kernOrisig = 20;
    ...
    kernOrisigMa = 15;
    kernXsigMa = 2;
    delayWin = [100 200];  %assume the peak response is within this time window
end
%%%%

kernAll_DS = cellS.kernAll; %Create a dummy version so that I don't downsample the global every time I run this.
kernCount_DS = cellS.kernCount;

%Downsample kernel and domains
%Downsample kernel and domains
oriDS = 1;
xDS = 1;
%if ~isempty(find(isnan(kernAll_DS{1})))
    for p = 1:length(kernAll_DS)
        
        if xDS
            id = find(isnan(kernAll_DS{p}));
            kernAll_DS{p}(id) = 0;

            kernA = kernAll_DS{p}(:,1:2:end,:,:,:,:);
            kernB = kernAll_DS{p}(:,2:2:end,:,:,:,:);

            countA = kernCount_DS{p}(:,1:2:end,:,:,:,:);
            countB = kernCount_DS{p}(:,2:2:end,:,:,:,:);

            kernAll_DS{p} = kernA.*countA + kernB.*countB; %"un-normalize"
            kernAll_DS{p} = kernAll_DS{p}./(countA + countB);  %downsample and renormalize

            kernCount_DS{p} = countA + countB;
...
for c = 1:length(DM.colordom)
    for p = 1:length(kernAll_DS)
        kernC{c}{p} = squeeze(kernAll_DS{p}(:,:,:,c,:));
    end
end
...
for c = 1:length(DM.colordom)
    ...
    for p = 1:Ncell
       ...
       subplot(ceil(sqrt(Ncell)),ceil(sqrt(Ncell)),p)
        ...
        kernplot_w(find(isnan(kernplot_w))) = nanmedian(kernplot_w(:));
        kernplot_b(find(isnan(kernplot_b))) = nanmedian(kernplot_b(:));
        ...
        for q = 1:3
            ...
            %if q == 1
                thresh2 = prctile(RF(:),20) + (max(RF(:))-prctile(RF(:),20))*.5;
            ...
            switch q
                ...
                case 1   %only plot the on+off
                    imagesc(xdom,ydom,RF,[0 1]), colormap jet%title(num2str(param(5)))
                    hold on
                ...
                case 2
%                     hold on
%                     contour(xdom,ydom,ffit,[cT cT],'k'), colormap jet
%                     axis image
%                     drawnow
                case 3
%                     hold on
%                     contour(xdom,ydom,ffit,[cT cT],'w')
%                     axis image
%                     drawnow
...
%             if q == 1
%                 if varaccount > 0.7
%                 contour(xdom,ydom,RF,[.61*max(RF(:)) .61*max(RF(:))],'k'), axis ij
%                 hold on
%             %     ylim([1.5 3.8])
%             %     xlim([.2 2.3])
%                 drawnow
