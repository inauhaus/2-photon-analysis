...
for c = 1:length(TC.opref)  %length(DM.colordom) might be different 
    ...
    PW.dsfBWLin{c} = zeros(1,Npair);
    PW.Dist{c} = zeros(1,Npair); PW.ax{c} = zeros(1,Npair);
    PW.doripair{c} = zeros(Npair,2); 
    PW.dorisigpair{c} = zeros(Npair,2); 
    PW.dsfpair{c} = zeros(Npair,2);     
    PW.dBWLinpair{c} = zeros(Npair,2); 
    ...
    PW.dLPnesspair{c} = zeros(Npair,2);
    ...
    for i = 1:MK.Ncell
        ...
        for j = (i+1):MK.Ncell
            ...
            PW.dsfBWLin{c}(k) = (log2(TC.sfBWLin{c}(i)/TC.sfBWLin{c}(j)));  %octaves
            PW.dphase{c}(k) = angle(exp(1i*pi/180*(TC.phase{c}(i)-TC.phase{c}(j))))*180/pi; %degrees; this one is useless cuz it uses 
            PW.dF1F0{c}(k) = TC.F1F0{c}(i) - TC.F1F0{c}(j); 

            PW.doripair{c}(k,:) = [TC.opref{c}(i) TC.opref{c}(j)];  %useful to have the oris for later
            PW.dorisigpair{c}(k,:) = [TC.orisig{c}(i) TC.orisig{c}(j)];  %useful to have the oris for later
            PW.dsfpair{c}(k,:) = [TC.sfpref{c}(i) TC.sfpref{c}(j)];  %useful to have the sf for later
            PW.dsfBWLinpair{c}(k,:) = [TC.sfBWLin{c}(i) TC.sfBWLin{c}(j)];  %(2sigma) useful to have later
            ...
            PW.dLPnesspair{c}(k,:) = [TC.LPness{c}(i) TC.LPness{c}(j)];
            ...
            %if the peak of phase tuning curve is near the peak of the
            %optimal ori/sf;  AND if the cell is responsive...
            if mean(tcphase_i) > .8*ma_i & mean(tcphase_j) > .8*ma_j & TC.SNR{c}(i)>1 & TC.SNR{c}(j)>1% & SNR_i>1 & SNR_j>1
                ...
                PW.dtcphasepair{c}(k,:) = [tcphase_i' tcphase_j'];  %useful to have the tuning curve for later
