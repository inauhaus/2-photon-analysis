...
global ACQinfo Tens1 Tens2 f0m1 f0m2 bcond pepANA TCWin
...
for(i=0:length(f0m1)-1)
    ...
    if(~pepblank)       %This loop filters out the blanks
        ...
        for z = 1:length(pepANA.listOfResults{i+1}.values)  %loop through each loop parameter
            %if strcmp(pepANA.listOfResults{i+1}.symbols(z),'t_period')
            if strcmp(pepANA.listOfResults{i+1}.symbols(z),'s_freq')
            %if strcmp(pepANA.listOfResults{i+1}.symbols(z),'radius')
            %if strcmp(pepANA.listOfResults{i+1}.symbols(z),'contrast')
                paramID = z;
                sfdom(i+1) = v(paramID);
            else
                pdumID = z;
...
subplot(2,2,1)
plot(sfdom1,tc1(id1),'-ok'), hold off
xlabel('spatial frequency'), title('Chan 1')
set(gca,'Xtick',round(sfdom1*1000)/1000,'Xscale','log')

subplot(2,2,2)
plot(sfdom2,tc2(id2),'-ok'), hold off
xlabel('spatial frequency'), title('Chan 2')
set(gca,'Xtick',round(sfdom2*1000)/1000,'Xscale','log')

nopix = length(yran)*length(xran);

subplot(2,2,3)
dum = squeeze(sum(sum(Tens1{idma1}(yran,xran,:),1),2))/nopix;
plot(tdom(1:end-1),dum(1:end-1),'-o')   %Get rid of last frame because shutter closes before the end
hold on
dum = squeeze(sum(sum(Tens1{idmi1}(yran,xran,:),1),2))/nopix;
plot(tdom(1:end-1),dum(1:end-1),'-or')
if isfield(ACQinfo,'stimPredelay')
