function [tcmat tcourseHi tcourseLo tcourseHi_var tcourseLo_var primDom blank legStr] = getpixeldata(pos,W)

global Tens Tens_var f0m f0m_var Analyzer symbolInfo
...
if Nsym == 3
    ...
    CollapseID = symbolInfo.Collapse(2)-1;
...
if Nsym > 1
    CollapseID = symbolInfo.Collapse(1) - 1;    
    
    if CollapseID == 0  %take mean
            ...
            tcmat = tcmat(:,CollapseID);
            tcourseArray = tcourseArray(:,CollapseID);
            tcourseArray_var = tcourseArray_var(:,CollapseID);
            
            legStr{2} = num2str(round(allDom{2}(CollapseID)*100)/100);
