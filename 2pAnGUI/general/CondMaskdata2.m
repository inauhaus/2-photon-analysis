...
DSflag = get(G_handles.DSflag,'Value');  % 2x2 binning flag
...
bwcell = maskS.bwCell{1};

if DSflag
    
    thresh = 0.5;
    bwcell = Tensor_2x2bin(bwcell); %2x2 binning
    idupper = find(bwcell>=thresh);
    idlower = find(bwcell<thresh);
    bwcell(idupper) = 1;
    bwcell(idlower) = 0;
    
end

masklabel = bwlabel(bwcell,4);

% if isfield(maskS,'maskTens')
%     masklabel = zeros(size(bwcell));
% else
%     masklabel = bwlabel(bwcell,4);
% end
