...
global ACQinfo bsflag repDom G_handles mbestall nbestall twophDATADIR maskS

GcampFlag = 1;

if get(G_handles.trialDeletion,'value')
    ZmovementDeletion = 1;
    deletionThresh = str2num(get(G_handles.deletionThreshold,'string'));
end
...
%             end 
        ...
        deleteBit = 0;
        if ZmovementDeletion
            
           imdum = mean(CH,3);
           dum = corrcoef(imdum(:),temp(:));
           rZ(t) = dum(1,2);
                
            if rZ(t) < deletionThresh
                deleteBit = 1;
                mbest = 0;
                nbest = 0;
            end
        ...
        if ~deleteBit %Only add good trials
            ...
            if varflag
                y2{c} = y2{c} + (CH.^2);  %to compute variance later
            end
